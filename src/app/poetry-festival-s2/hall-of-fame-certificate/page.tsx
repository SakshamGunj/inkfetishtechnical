...
import { Search, CheckCircle2, Download, Loader2, FileText, Star, Zap } from 'lucide-react';
...
export default function HallOfFameCertificatePage() {
  // Step Management
  const [step, setStep] = useState<'search' | 'success' | 'loading-promo' | 'promo'>('search');
  ...
  const handleDownloadClick = async (e: React.MouseEvent) => {
    ...
    if (selectedUser) {
      try {
        const url = `https://fmnnomndxnybjsbykpbr.supabase.co/storage/v1/object/public/halloffamepf/${selectedUser.file}`;
        ...
        a.href = downloadUrl;
        a.download = `Hall_of_Fame_Certificate_${selectedUser.name.replace(/\\s+/g, '_')}.png`;
        ...
        window.open(`https://fmnnomndxnybjsbykpbr.supabase.co/storage/v1/object/public/halloffamepf/${selectedUser.file}`, '_blank');
      }
    }
  };
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setIsSearching(true);
      try {
        const res = await fetch(`/api/hall-of-fame-certificate/search?q=${encodeURIComponent(query)}`);
      ...
      <div className="flex-1 max-w-2xl w-full mx-auto px-4 sm:px-6 pt-12 md:pt-20 pb-12 md:pb-20 flex flex-col justify-center">
        ...
        <div className="text-center mb-8 md:mb-12">
          ...
          <h1 className="text-2xl md:text-4xl font-serif font-black uppercase tracking-tight text-[#fdfbf7] mb-2 md:mb-3">
            Hall of Fame <span className="italic font-light text-gold">Top 200</span>
          </h1>
          <p className="text-gold text-xs md:text-sm tracking-[0.2em] font-bold uppercase mb-4">
            Official Certificate Retrieval
          </p>
          <p className="text-[#a090b0] text-sm max-w-md mx-auto leading-relaxed px-2">
            Search your name to instantly claim and download your official Poetry Festival Season 2 Hall of Fame Certificate.
          </p>
        </div>
          ...
          <AnimatePresence mode="wait">
                      ...
                      >
                        {searchResults.map((user) => (
                          ...
                          >
                            ...
                            <span className="text-xs md:text-sm text-gold/80 font-serif tracking-wider opacity-0 group-hover:opacity-100 transition-opacity">
                              Select
                            </span>
                          </button>
                        ))}
            ...
            {step === 'success' && selectedUser && (
              ...
              >
                ...
                <h2 className="text-2xl font-serif text-white mb-2 font-bold">Congratulations!</h2>
                <p className="text-sm text-[#888] mb-6">
                  <strong className="text-white">{selectedUser.name}</strong>, your official Hall of Fame Certificate is ready.
                </p>
                ...
                <div className="p-4 bg-[#121212] border border-gold/20 rounded-md mb-6">
                  ...
                  <p className="text-lg font-serif text-white mb-4">Top 200 Certificate</p>
                  
                  {/* Image Preview */}
                  <div className="relative w-full max-w-lg mx-auto bg-black/50 rounded-md overflow-hidden border border-white/10 mb-4 shadow-inner group flex justify-center items-center min-h-[200px]">
                    <img 
                      src={`https://fmnnomndxnybjsbykpbr.supabase.co/storage/v1/object/public/halloffamepf/${selectedUser.file}`}
                      className="max-w-full h-auto object-contain z-10"
                      alt="Hall of Fame Certificate"
                    />
                  </div>
            ...
            {step === 'loading-promo' && (
              ...
              >
                ...
                <h2 className="text-2xl md:text-3xl font-serif text-white font-bold mb-3">Fetching Your Official Certificate...</h2>
                <p className="text-[#888] max-w-sm mx-auto">Please wait a moment while we securely generate and save your Certificate to your device.</p>
              </motion.div>
            )}

            {/* STEP 3: PROMO / UPSSELL - HORMOZI STYLE */}
            {step === 'promo' && (
              <motion.div
                key="step-promo"
                initial={{ opacity: 0, scale: 0.95 }}
              ...
              >
                ...
                <div className="text-center mb-10">
                  ...
                  <p className="text-[#bbb] text-sm md:text-lg max-w-3xl mx-auto leading-relaxed mb-6 sm:mb-8 px-4">
                    You've already proven you are among the elite. Whether your poems are perfectly polished or just sitting in your notes app, we will guide you step-by-step. We handle the heavy lifting so you can finally get the fame, respect, and recognition you deserve. 
