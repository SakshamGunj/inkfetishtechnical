// Mock Date to bypass 2026 local time issue
const REAL_TIME_OFFSET = new Date('2026-08-29T13:17:20Z').getTime() - new Date('2024-08-29T13:17:20Z').getTime();

const OriginalDate = Date;
global.Date = class extends OriginalDate {
  constructor(...args: any[]) {
    if (args.length === 0) {
      super(OriginalDate.now() - REAL_TIME_OFFSET);
    } else {
      // @ts-ignore
      super(...args);
    }
  ...
  static now() {
    return OriginalDate.now() - REAL_TIME_OFFSET;
  }
} as DateConstructor;
