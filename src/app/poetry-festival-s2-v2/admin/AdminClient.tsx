...
export default function AdminClient() {
  ...
  const fetchSubmissions = async () => {
    ...
    try {
      ...
      alert('Error fetching submissions from Supabase. Ensure table exists and RLS is disabled.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadPdf = async () => {
    if (!selectedSubmission || !paperRef.current) return;
    setIsCompiling(true);
    try {
      const filename = `${selectedSubmission.authorName.replace(/\s+/g, '_')}_A5_Submission.pdf`;
  ...
  const handleMasterDownload = async () => {
    ...
    if (submissions.length === 0) {
      alert("No submissions to compile!");
      return;
    }
    
    setIsCompiling(true);
    try {
      const filename = `Master_Anthology_${new Date().toISOString().split('T')[0]}.pdf`;
      await generatePdfFromElement(masterContainerRef.current, filename);
    } catch (e) {
      console.error(e);
      alert('Error compiling master PDF.');
    } finally {
      setIsCompiling(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white flex flex-col md:flex-row font-sans overflow-hidden">
      ...
      <div className={`w-full md:w-80 shrink-0 border-r border-white/10 flex-col h-screen bg-[#0a0a0a] z-20 ${selectedSubmission ? 'hidden md:flex' : 'flex'}`}>
        ...
        <div className="p-4 border-b border-white/10 shrink-0 bg-[#111] space-y-2">
          <button 
            onClick={handleMasterDownload}
            disabled={isCompiling || submissions.length === 0}
            className="w-full py-2.5 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold uppercase tracking-widest flex items-center justify-center gap-2 rounded-sm transition-colors disabled:opacity-50"
          >
            {isCompiling ? <Loader2 className="w-4 h-4 animate-spin" /> : <BookOpen className="w-4 h-4" />}
            Compile Master Book PDF
          </button>
        </div>
          ...
          <>
            ...
            <div className="flex-1 overflow-auto bg-[#0f0f0f] custom-scrollbar relative">
              ...
              <div className="min-w-[600px] p-4 sm:p-10 flex flex-col items-center justify-start gap-12 absolute inset-0">
                <div ref={paperRef} className="pdf-capture-container bg-transparent shrink-0">
                
                {/* 
                  We structure this EXACTLY like the front-end submission container.
                  The wrapper forces standard A5 dimensions for the export.
                */}
                <div 
                ...
                >
                  {/* Header matches front-end */}
                  <div className="flex flex-col items-end space-y-1 shrink-0 mb-4">
                    <div className="w-full font-serif text-xl font-bold text-right">
                      {selectedSubmission.title}
                    </div>
                    <div className="flex items-center justify-end gap-2 w-full">
                      <span className="font-serif italic text-black/60 text-sm">By</span>
                      <div className="font-serif italic text-sm text-right w-[350px]">
                        {selectedSubmission.authorName}
                      </div>
                    </div>
                  </div>

                  {/* The Content (Parsed from TipTap HTML) */}
                  <div 
      ...
      {/* ── INVISIBLE MASTER PDF COMPILER CONTAINER ── */}
      <div 
        className="absolute top-0 left-0 opacity-0 pointer-events-none -z-50" 
        style={{ width: '559px' }}
      >
        <div ref={masterContainerRef} className="bg-transparent">
          {submissions.map((sub, index) => (
            <div key={sub.id}>
              {/* 
                Each submission is forced into an A5 container. 
                We use page-break-after: always on the wrapper so html2pdf cuts it properly.
              */}
              <div 
                className="bg-white text-black relative mx-auto"
                style={{
              ...
              >
                ...
                <div 
                  className="parsed-tiptap-content"
                  style={{ wordWrap: 'break-word' }}
                  dangerouslySetInnerHTML={{ __html: sub.poetryHtml }}
                />
              </div>
            </div>
          ))}
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        /* Make sure the TipTap HTML renders exactly like the editor */
        .parsed-tiptap-content p {
          margin-bottom: 0;
          min-height: 1.5em; 
        }
