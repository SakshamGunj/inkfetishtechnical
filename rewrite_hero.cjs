...
const heroRegex = /<div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10 flex flex-col lg:flex-row items-center lg:items-start lg:justify-between gap-12 lg:gap-8">([\s\S]*?)<\/section>/;

const newHero = `<div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-x-8 lg:gap-y-0 items-center lg:items-start">
          
          {/* TITLE SECTION (Top left on desktop, Top on mobile) */}
          ...
          {/* IMAGE SECTION (Right on desktop, Middle on mobile) */}
          <div className="lg:col-span-6 lg:row-span-2 flex justify-center lg:justify-end lg:-mt-4 w-full">
            <motion.div 
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              transition={{ duration: 0.8, ease: "easeOut" }}
              className="relative rounded-sm overflow-hidden border-2 border-gold/40 bg-[#1A1613] shadow-[0_20px_50px_rgba(197,160,89,0.15)] p-2 w-full max-w-[280px] sm:max-w-sm lg:max-w-xl mx-auto lg:mx-0"
            >
          ...
          {/* CTA SECTION (Bottom left on desktop, Bottom on mobile) */}
          <div className="lg:col-span-6 flex flex-col items-center lg:items-start w-full mt-2 lg:mt-8">
            <motion.div 
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
...
console.log('Hero section rewritten with Grid successfully.');
