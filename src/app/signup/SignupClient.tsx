...
const SignupClient = () => {
    ...
    const handleSignup = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 2. Initialize their Portfolio Document in Firestore with status: 'pending' and approved: false
            ...
            // 3. Immediately sign out user so they are NOT auto-logged in
            await auth.signOut();
                ...
                <div className="relative z-10">
                    {isSubmitted ? (
                        <div className="space-y-6 text-center py-4">
                            <div className="w-16 h-16 bg-[#FFC700] border-[3px] border-black mx-auto flex items-center justify-center text-black text-3xl font-black shadow-[4px_4px_0_0_#000] rotate-3">
                                ⏳
                            </div>
                            <h1 className="text-3xl md:text-4xl font-black uppercase tracking-tighter text-black">
                                APPLICATION SUBMITTED!
                            </h1>
                            <div className="bg-[#FFC700]/20 border-[3px] border-black p-4 text-left font-mono space-y-2">
                                <p className="text-xs font-black text-gray-600 uppercase">RESERVED CALLSIGN:</p>
                                <p className="font-bold text-sm md:text-base text-black break-all bg-white border-2 border-black p-2">
                                    inkfetish.com/author/{submittedUsername}
                                </p>
                            </div>
                            <p className="text-gray-700 font-bold text-sm md:text-base leading-relaxed border-l-4 border-black pl-3 text-left uppercase">
                                Your author profile is currently <span className="bg-black text-white px-2 py-0.5">PENDING ADMIN APPROVAL</span>. Our editorial team will review your application and make your site live shortly.
                            </p>
                            <div className="pt-4">
                                <Link 
                                    href="/login" 
                                    className="block w-full bg-black text-white border-[3px] border-black py-4 font-black text-lg uppercase tracking-widest hover:bg-[#39FF14] hover:text-black hover:-translate-y-1 hover:shadow-[4px_4px_0_0_#000] active:translate-y-0 active:shadow-none transition-all duration-200"
                                >
                                    GO TO LOGIN PAGE
                                </Link>
                            </div>
                        ...
                        <>
                            ...
                            <form onSubmit={handleSignup} className="space-y-5">
                                ...
                                >
                                    {isLoading ? 'STARTING...' : 'SUBMIT APPLICATION'}
                                </button>
                            </form>
