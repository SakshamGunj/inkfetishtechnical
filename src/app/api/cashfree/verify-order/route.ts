...
// Use service-role key for server-side writes if available, else fall back to anon key
...
export async function GET(request: Request) {
  ...
  try {
    // Fetch order status directly from Cashfree — the authoritative source
    const response = await fetch(`${baseUrl}/pg/orders/${orderId}`, {
      ...
      // Vercel: disable cache so we always get fresh status
      cache: 'no-store',
    });
