                        ...
                        <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
                            ...
                            >
                                ...
                                <ul className="space-y-3 mb-6 text-[#5D4037] text-sm text-left px-2">
                                    <li className="flex items-start gap-2"><Check className="w-4 h-4 text-green-600 mt-1 flex-shrink-0" /> <span>Submit 1 writing (any one category)</span></li>
                                    <li className="flex items-start gap-2"><Check className="w-4 h-4 text-green-600 mt-1 flex-shrink-0" />
                                        <span>Get 2 government-verified e-certificates:<br /><span className="text-xs text-[#8D6E63] pl-1">• Participation Certificate<br />• Writer’s Excellence Certificate</span></span>
                                    </li>
                                </ul>
                            ...
                            >
                                ...
                                <ul className="space-y-3 mb-6 text-[#E5D4B3] text-sm text-left px-2">
                                    <li className="flex items-start gap-2"><Star className="w-4 h-4 text-[#FFD700] mt-1 flex-shrink-0" /> <span>Submit 2 writings (same or different categories)</span></li>
                                    <li className="flex items-start gap-2"><Star className="w-4 h-4 text-[#FFD700] mt-1 flex-shrink-0" /> <span>Get physical certificates delivered to your home 📬</span></li>
                                    <li className="flex items-start gap-2"><Check className="w-4 h-4 text-[#FFD700] mt-1 flex-shrink-0" /> <span>Receive a winner-style medal 🏅</span></li>
                                    <li className="flex items-start gap-2"><Check className="w-4 h-4 text-[#FFD700] mt-1 flex-shrink-0" /> <span>Get an official appreciation letter 📝</span></li>
                                </ul>
...
const IndianWritersLeague = () => {
    ...
    useEffect(() => {
        ...
        // Check URL first, then fall back to saved orderId (robustness against refresh)
        const orderId = urlParams.get('order_id') || formData.orderId;

        // If we have an orderId, we should check status
        if (orderId) {
            // Skip intro if returning from payment
            setAnimationState('form');

            // Optimization: If we already think we are paid (e.g. from local storage flag if we added one, currently we don't), 
            // we should still maybe verify or just trust if we want super speed. 
            // Better to verify.

            if (paymentStatus === 'pending') {
                const verifyPayment = async (retries = 5) => {
                    ...
                    try {
                        ...
                        if (data.order_status === 'PAID') {
                            ...
                            // Determine Plan from Amount (Production)
                            const paidAmount = Number(data.order_amount);
                            const verifiedPlan = paidAmount >= 499 ? 499 : 299;
                            ...
                            setFormData(prev => {
                                // 1. Try to recover data from localStorage 
                                let recoveredData = {};
                                try {
                                    const saved = localStorage.getItem('iwl_registration');
                                    if (saved) recoveredData = JSON.parse(saved);
                            ...
                            // If status is PENDING or USER_DROPPED, we might want to retry if PENDING
                            if (data.order_status === 'PENDING' && retries > 0) {
                                console.log(`Payment Pending. Retrying... (${retries} attempts left)`);
                                setTimeout(() => verifyPayment(retries - 1), 2000); // 2s delay
                                return;
                            }

                            console.warn("Payment failed or final status not PAID.", data);
                            setPaymentStatus('pending');
                            if (urlParams.get('order_id')) {
                                console.error("Payment Verify Failed:", data);
                                alert(`Payment verification failed. Status: ${data.order_status || "Unknown"}\nMessage: ${data.message || "No message"}`);
                                window.history.replaceState({}, document.title, window.location.pathname);
                                setShowPlans(true);
                            }
                        }
                        ...
                        if (retries > 0) {
                            console.log(`Verification Error. Retrying... (${retries} attempts left)`);
                        ...
                        setPaymentStatus('pending');
                        if (urlParams.get('order_id')) {
                            alert("Payment verification error (Network). Please try again.");
                            window.history.replaceState({}, document.title, window.location.pathname);
                            setShowPlans(true);
                        }
                    }
    ...
    }, [formData.orderId, paymentStatus]); // Dependencies
    ...
    const createInitialRecord = async (orderId: string, currentData: FormData) => {
        ...
        // Ensure we don't overwrite if title exists? No, this is initial record.
        // If they have submissions, we shouldn't overwrite them?
        // Upsert will update provided fields. `submission_1_title` etc are separate columns.
        // So updating name/email is safe.
        ...
        await supabase.from('iwl_registrations').upsert({
            ...
            payment_status: 'paid',
            // Only update category if we have it, otherwise ignore (postgres handles null in upsert update?)
            // Actually if we pass null, it overwrites.
            // But usually this runs on dashboard load where category might be null or set.
            // Let's safe guard category.
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            */
            ...
            // Update 'Submitted' visual state locally? 
            // We already have state. Just transition to Success.

            // Note: Unlike before, we don't clear localStorage completely because they might have entry 2 to submit.
            // But we should maybe mark it? The components check `formData.submission1.title`.
    ...
    return (
        <div className={`min-h-screen font-sans selection:bg-[#420C0C] selection:text-[#FFD700] ${showCinematic ? 'bg-[#1A0505] text-[#FFD700]' : 'bg-[#F0EBE0]'}`}>
            <Helmet>
                <title>Indian Writers League | Season 1</title>
            ...
            <AnimatePresence mode="wait">
                ...
                {/* Step 2 (Plan Selection) is now merged into Step 1. */}
...
export default IndianWritersLeague;
